#!/bin/bash
make
valgrind --show-leak-kinds=all --leak-check=full --track-fds=yes --verbose --show-mismatched-frees=yes --read-var-info=yes --suppressions=_ignore_leaks.txt --log-file=valgrind-out.log ./minishell
