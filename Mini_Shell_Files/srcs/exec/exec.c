...
void	make_child(t_mini_shell *ms, t_cmd *cmd, int is_last)
{
	if (cmd->output->fd == -1)
	{
	...
	if (cmd->output->fd == -2 && is_last)
		cmd->output->fd = STDOUT_FILENO;
	else if (cmd->output->fd == -2)
...
t_error	exec_cmds(t_mini_shell *ms)
{
	...
	{
		cmd = get(current);
		if (pipe_cmd_fd(ms, cmd, !current->previous) == ERROR)
		{
			current = current->next;
			continue;
		}
		dprintf(2, "\n\nbefore MAKE CHILD\n\n");
		make_child(ms, cmd, !current->next);
		dprintf(2, "AFTER MAKECHILD : PID=%d\n", cmd->pid);
		if (!cmd->pid)
			exec_cmd(ms, cmd);
		current = current->next;
	}
