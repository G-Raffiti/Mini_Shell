...
/*   Updated: 2023/03/27 17:58:59 by aucaland         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (which_env == 1 && cmd->cmd && cmd->cmd[1] && \
								ft_str_cmp(cmd->cmd[1], "PATH") == 0)
...
t_error	remove_env_unset(t_mini_shell *ms, t_cmd *cmd, \
							t_lstd *current, t_lstd *current_sorted)
{
	if (current)
	{
		ms->env_dict = ft_lstd_first(ft_lstd_remove_and_del(current, \
													(void *)free_dict));
		if (fill_env_unset(ms, cmd, 1) == MALLOC_ERROR)
			return (MALLOC_ERROR);
	}
	if (current_sorted)
	{
		...
		if (fill_env_unset(ms, cmd, 2) == MALLOC_ERROR)
			return (MALLOC_ERROR);
	}
	return (SUCCESS);
}
...
t_error	ft_unset(t_mini_shell *ms, t_cmd *cmd, int in_pipe)
{
	...
	i = 0;
	while (!in_pipe && cmd->cmd[i] && cmd->cmd[++i] && \
							ft_str_cmp(cmd->cmd[i], "_") != 0)
	{
		...
		if (remove_env_unset(ms, cmd, current, current_sorted) == MALLOC_ERROR)
			return (MALLOC_ERROR);
	}
